...
$sql_total="SELECT ID FROM `datos_comentarios` WHERE LEIDO='NO'";
...
$consulta="SELECT `datos_comentarios`.ID AS ID, `datos_comentarios`.NOMBRE AS NOMBRE_COMENTARISTA, `datos_comentarios`.CORREO AS CORREO_COMENTARISTA, `datos_comentarios`.ART_TITULO AS ARTICULO, `datos_comentarios`.FECHA AS FECHA_COMENTARIO, `datos_comentarios`.LEIDO AS LEIDO, `datos_comentarios`.COMENTARIO AS COMENTARIO, `datos_usuarios`.NOMBRE AS NOMBRE_AUTOR, `datos_usuarios`.APELLIDO AS APELLIDO_AUTOR, `datos_articulos`.ART_FECHA AS FECHA_ARTICULO FROM `datos_comentarios` INNER JOIN `datos_articulos` ON `datos_comentarios`.ART_TITULO = `datos_articulos`.ART_TITULO INNER JOIN `datos_usuarios` ON `datos_articulos`.AUTOR_CORREO = `datos_usuarios`.AUTOR_CORREO WHERE LEIDO='NO' ORDER BY NOMBRE_AUTOR ASC, APELLIDO_AUTOR ASC, ARTICULO ASC, FECHA_COMENTARIO ASC LIMIT $empesar_desde,$registros_filtrados";
$resultado=mysqli_query($conexion,$consulta);
$i=0;
while(($fila=mysqli_fetch_array($resultado))==true){
	$id[$i]=$fila['ID'];
    ...
    <table>
        <tr>
            ...
            while($e<$i){
				...
				echo "<td style='border:#000 1px solid; width:10%;'><form action='rud_marcar_leido_nl.php' method='post'><input type='hidden' name='id' value='$id[$e]'><input type='hidden' name='pagina' value='$pagina'><input type='submit' name='leido' style='background-color:#8f8;' value='Marcar como Leido'></form></td>";
				
                echo "<td style='border:#000 1px solid; width:8%;'><form action='rud_responder_comentario_nl.php' method='post'><input type='hidden' name='id' value='$id[$e]'><input type='hidden' name='pagina' value='$pagina'><input type='submit' name='responder' value='Responder'></form></td>";
                echo "<td style='border:#000 1px solid; width:8%;'><form action='delete_rud_comentarios_nl.php' method='post'><input type='hidden' name='id' value='$id[$e]'><input type='submit' name='borrar' value='Borrar'></form></td>";
                echo "</tr>";
                $e=$e+1;
            }
            ?>
