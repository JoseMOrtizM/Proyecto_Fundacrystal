...
//FECHA PARA LA QUE SE ACTUALIZÓ ESTA PAGINA
$fecha_ahora_ano=date("Y"); 
$fecha_ahora_mes=date("m");

//OBTENIENDO EL AÑO Y EL MES DEL FORM
if(!isset($_POST['hemeroteca'])){
	$hemeroteca="Año: $fecha_ahora_ano / Mes: $fecha_ahora_mes ()";
}else{
	if($_POST['hemeroteca']=="Seleccione un Mes"){
		$hemeroteca="Año: $fecha_ahora_ano / Mes: $fecha_ahora_mes ()";
	}else{
		$hemeroteca=$_POST["hemeroteca"];
	}
}
if(isset($_GET['hemeroteca'])){
  $hemeroteca=$_GET["hemeroteca"];
}

//PARTIENDO EL DATO
$porc_01=explode("Año: ", $hemeroteca);
$porc_02=$porc_01[1];
$porc_03=explode(" / Mes: ", $porc_02);
$ano_form=$porc_03[0];
$porc_04=$porc_03[1];
$porc_05=explode(" (", $porc_04);
$mes_form=$porc_05[0];
?>
<!DOCTYPE HTML>
<html>
...
echo "<link rel='canonical' href='http://www.fundacrystal.tk/hemeroteca.php' />";
...
?>
<title>Fundación Crystal - Hemeroteca</title>
...
<?php
	...
	$sql_total="SELECT `datos_articulos`.ID AS ID FROM `datos_articulos` INNER JOIN `datos_usuarios` ON `datos_articulos`.AUTOR_CORREO=`datos_usuarios`.AUTOR_CORREO WHERE `datos_articulos`.ART_ANO = '$ano_form' AND `datos_articulos`.ART_MES = '$mes_form'";
...
<?php
    ...
    $consulta="SELECT 
				...
				`datos_articulos`.ART_ANO = '$ano_form' AND `datos_articulos`.ART_MES = '$mes_form'
    ...
    <article style="background-color:#376092;">
      <table style="width:auto; margin:auto;">
        <tr>
          <td>
          <?php
          //ESTA ES LA VARIABLE PARA DEVOLVER HEMEROTECA
          $imprimir_hemeroteca="Año: " . $ano_form . " / Mes: " . $mes_form . " (0)";
          if($pagina>1){
			    $pag_ant=$pagina-1;
		  		echo "<a href='?pag_form=$pag_ant&hemeroteca=$imprimir_hemeroteca' style='color:#EE5;'>< Página Anterior</a>&nbsp;&nbsp;&nbsp;";   
		  }else{}

		  if($pagina<>$total_paginas){
			  	$pag_sig=$pagina+1;
		  		echo "&nbsp;&nbsp;&nbsp;<a href='?pag_form=$pag_sig&hemeroteca=$imprimir_hemeroteca' style='color:#EE5;'>Págna Siguiente ></a>";
		  }else{}
