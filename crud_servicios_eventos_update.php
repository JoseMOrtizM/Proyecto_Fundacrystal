...
<head>
<meta charset="utf-8">
<title>Fundación Crystal - CRUD/Update Servicios</title>
...
if($_FILES['nombre_nueva_imagen']['name']<>true){
	$img_name=$_POST['foto_servicio_nombre'];
	echo "$img_name";
	$img_type='';
	$img_size=0;
	$ruta_temporal='';
...
if($ver_act=='ok' and $ver_sub_act=='ok' and $ver_fecha=='ok' and $ver_titulo=='ok' and $ver_descripcion=='ok' and $ver_img_type=='ok' and $ver_img_size=='ok'){
	...
	$consulta="UPDATE `datos_servicios` SET 
	ACT='$act_nombre',
	SUB_ACT='$sub_act_nombre',
	TITULO='$sub_act_titulo',
	DESCRIPCION='$sub_act_descripcion',
	FOTO='$img_name',
	FECHA='$sub_act_fecha'
	WHERE ID='$id';";
	$resultado=mysqli_query($conexion,$consulta);
	...
	echo "<table style='border: solid #333 1px; width:97%; margin:auto; background-color:#dfdfdf;'><tr style='border: solid #333 1px;'><td colspan='4' style='border: solid #333 1px; width:15%; text-align:justify; padding:5px; background-color:#FFF; color:#F00;'><h3>DATOS ACTUALIZADOS CON ÉXITO</h3></td></tr></table>";
}else{
